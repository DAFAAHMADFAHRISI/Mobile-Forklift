...
class PesananService {
  static const String baseUrl = 'http://10.251.130.195:3000/api/pesanan';
  ...
  static Future<Map<String, dynamic>?> submitPesanan({
    ...
    final response = await http.post(
      Uri.parse('http://10.251.130.195:3000/api/pesanan/store'),
      headers: {
        'Content-Type': 'application/json',
  ...
  void startPolling(
    ...
    _timer = Timer.periodic(Duration(seconds: 5), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('http://10.251.130.195:3000/api/payment/status/$orderId'),
          headers: {
            'Content-Type': 'application/json',
            if (token != null) 'Authorization': 'Bearer $token',
          },
